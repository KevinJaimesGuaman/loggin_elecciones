    ...
    class administrador_CrearEditar : AppCompatActivity() {
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            val spinnerTipoEleccion = findViewById<AutoCompleteTextView>(R.id.spinnerTipoEleccion)
            ...
            val imagenMas = findViewById<ImageView>(R.id.imagenMas)
            imagenMas.setOnClickListener {
                val tipoEleccion = spinnerTipoEleccion.text.toString()

                val intent: Intent
                if (tipoEleccion.equals("Votacion para Rector", ignoreCase = true)) {
                    // Iniciar add_partido_rector
                    intent = Intent(this, add_partido_rector::class.java)
                } else {
                    // Iniciar add_partido
                    intent = Intent(this, add_partido::class.java)
                }

                // Pasar el texto del spinner al siguiente Activity (opcional)
                intent.putExtra("TIPO_ELECCION", tipoEleccion)
                startActivity(intent)
            }
